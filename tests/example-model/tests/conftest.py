...
from oarepo_model_builder_multilingual.resources.records.config import (
    OarepoModelBuilderMultilingualResourceConfig,
)

BASE_URLS = {
    "base_url": OarepoModelBuilderMultilingualResourceConfig.url_prefix,
    "base_html_url": "/oarepo-model-builder-multilingual/",
}
...
def sample_record(app, db, input_data):
    ...
    with UnitOfWork(db.session) as uow:
        record = OarepoModelBuilderMultilingualRecord.create(input_data)
...
def sample_records(app, db, sample_metadata_list):
    ...
    with UnitOfWork(db.session) as uow:
        ...
        for sample_metadata in sample_metadata_list:
            record = OarepoModelBuilderMultilingualRecord.create(sample_metadata)
            uow.register(RecordCommitOp(record, current_service.indexer, True))
